                        ...
                        {!org ? (
                            <Card className="border-white/5 bg-white/[0.02] backdrop-blur-3xl rounded-[32px] overflow-hidden p-12 text-center space-y-8 border-0 shadow-2xl">
                                ...
                                <div className="space-y-4">
                                    <h2 className="text-3xl font-black text-white">Setup Your Organization</h2>
                                ...
                                {user?.plan === 'organization' ? (
                                    <div className="max-w-md mx-auto space-y-4">
                                        <Input 
                                            ...
                                            className="h-14 bg-black/40 border-white/10 rounded-2xl text-lg font-bold text-white placeholder:text-slate-600 focus:border-primary/50"
                                        />
                                        <Button 
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <Button variant="outline" className="h-12 px-8 rounded-xl border-white/10 hover:bg-white/5" asChild>
                            ...
                            <div className="grid grid-cols-1 gap-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <Card className="md:col-span-2 border-white/5 bg-white/[0.02] backdrop-blur-xl rounded-3xl border-0">
                                        <CardHeader className="p-8 pb-4">
                                            <CardTitle className="text-2xl font-black text-white flex items-center gap-3">
                                                {org.name}
                                                <Badge className="bg-primary/20 text-primary border-primary/20">Organization</Badge>
                                            </CardTitle>
                                            <CardDescription className="text-slate-400 font-medium">Owned by {org.owner_name}</CardDescription>
                                        </CardHeader>
                                        <CardContent className="p-8 pt-4 space-y-6">
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="p-6 rounded-2xl bg-white/5 border border-white/5 space-y-2">
                                                    ...
                                                    <div className="text-3xl font-black text-white">{members.length} / {org.max_members}</div>
                                                </div>
                                                <div className="p-6 rounded-2xl bg-white/5 border border-white/5 space-y-2">
                                                    ...
                                                    <div className="text-xl font-black text-white">Active (Expert)</div>
                                                </div>
                                            </div>
                                        </CardContent>
                                    </Card>
                                    ...
                                    {isOwner && (
                                        <Card className="border-white/5 bg-primary/5 backdrop-blur-xl rounded-3xl border-0 flex flex-col justify-center p-8 space-y-6">
                                            ...
                                            <div className="space-y-3">
                                                <div className="relative">
                                                    ...
                                                    <Input 
                                                        ...
                                                        className="h-12 bg-black/40 border-white/10 rounded-xl pl-10 text-sm"
                                                    />
                                                </div>
                                                <div className="grid grid-cols-1 gap-2">
                                                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Select Access Tier</span>
                                                    <Select value={selectedPlan} onValueChange={(v: any) => setSelectedPlan(v)}>
                                                        <SelectTrigger className="h-12 bg-black/40 border-white/10 rounded-xl font-bold">
                                                            <SelectValue />
                                                        </SelectTrigger>
                                ...
                                {/* Members Table */}
                                <Card className="border-white/5 bg-white/[0.02] backdrop-blur-xl rounded-3xl overflow-hidden border-0">
                                    <CardHeader className="p-8 border-b border-white/5">
                                        <CardTitle className="text-xl font-black text-white">Team Members</CardTitle>
                                    </CardHeader>
                                    <Table>
                                        <TableHeader className="bg-white/[0.01]">
                                            <TableRow className="border-white/5 hover:bg-transparent">
                                                <TableHead className="py-5 pl-8 text-[10px] font-black uppercase tracking-widest text-slate-400">User</TableHead>
                                        ...
                                        <TableBody>
                                            {members.map((member) => (
                                                <TableRow key={member.id} className="border-white/5 hover:bg-white/[0.01] transition-colors">
                                                    <TableCell className="py-5 pl-8">
                                                        <div className="flex items-center gap-4">
                                                            <div className="size-10 rounded-full bg-slate-800 border border-white/10 overflow-hidden">
                                                    ...
                                                    <TableCell>
                                                        {member.id === org.owner_id ? (
                                                            ...
                                                            <Badge variant="outline" className="border-white/10 text-slate-400">Member</Badge>
                                                        )}
                                                    </TableCell>
                                                        ...
                                                        {isOwner && member.id !== user?.id && (
                                                            <div className="flex items-center justify-end gap-2">
                                                                <DropdownMenu>
                                                                    ...
                                                                    <DropdownMenuContent align="end" className="bg-[#16181D] border-white/10 w-48">
                                                                        <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-widest text-slate-500">Subscription Plan</DropdownMenuLabel>
                                                                        <DropdownMenuItem 
                                                                            onClick={() => handleUpdateMemberPlan(member.id, 'pro')}
                                                                            className="flex items-center gap-2 font-bold focus:bg-white/5"
                                                                        >
